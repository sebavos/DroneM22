...
# MAGIA DESCUBIERTA: Comando Protobuf para H.265. 
# Como los puertos TCP están cerrados en el M22, se inyecta por UDP 8800.
# Formato crudo sin el prefijo de longitud TCP:
CMD_START_HEVC = bytes.fromhex("08 03 2A 04 08 02 10 02")
...
def control_loop(sock_udp):
    ...
    # 0. Autorizar cliente (Obligatorio para que el dron no bloquee el stream)
    print("[CONTROL] Registrando SSID del cliente...")
    try:
    ...
    # 2. Enviar el comando secreto Protobuf para cambiarlo a H.265 por UDP
    time.sleep(1.0)
    print(f"[CONTROL] Inyectando comando secreto H.265 (HEVC) vía UDP al puerto {CMD_PORT}...")
    try:
        # Enviar dos veces por si se pierde el paquete UDP
        sock_udp.sendto(CMD_START_HEVC, (DRONE_IP, CMD_PORT))
        time.sleep(0.1)
        sock_udp.sendto(CMD_START_HEVC, (DRONE_IP, CMD_PORT))
        print("[CONTROL] >>> ¡ÉXITO! Comando H.265 (HEVC) inyectado por UDP. <<<")
    except Exception as e:
        print(f"[CONTROL] Fallo UDP H.265: {e}")
...
if __name__ == '__main__':
    ...
    # Socket único para TX y RX (CRÍTICO: el dron responde al mismo puerto que envía CMD_REGISTER)
